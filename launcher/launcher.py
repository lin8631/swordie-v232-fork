...
SERVER_IP = "YOUR_DUCKDNS_SUBDOMAIN.duckdns.org"  # e.g. "myserver.duckdns.org"
API_PORT = 3000
GAME_PATH = r"C:\Path\To\Maplestory V232.2\MapleStory.exe"  # path to your game client

# Use localhost when running on the host machine, external hostname for friends
import socket
def _get_api_base():
    try:
...
class LoginFrame(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Button(card, text="Create Account", font=("Segoe UI", 9),
                  ...
                  command=lambda: self.parent.show_frame(RegisterFrame)).pack(fill="x")
        ...
        self.server_label = tk.Label(status_row, text=f"  Checking server...",
    ...
    def _check_server(self):
        try:
            ...
            if r.status_code == 200:
                ...
                self.after(0, lambda: self.server_label.config(text=f"  Server online — {SERVER_IP}", fg=GREEN))
            else:
                raise Exception()
        except:
            self.after(0, lambda: self.dot.config(fg=ACCENT))
            self.after(0, lambda: self.server_label.config(text=f"  Server offline or unreachable", fg=ACCENT))
    ...
    def _launch(self, token):
        ...
        try:
            ...
            self._error(f"MapleStory.exe not found at:\n{GAME_PATH}")
...
class RegisterFrame(tk.Frame):
    ...
    def _build(self):
        ...
        for label, attr, show in [
            ("Username", "username", ""),
            ("Password", "password", "•"),
            ("Email (optional — leave blank to skip)", "email", ""),
        ]:
